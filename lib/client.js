...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      '.whl-whale.whl-grip { cursor: grabbing; }',
      ...
      '.whl-whale-svg { display: block; filter: drop-shadow(0 7px 12px rgba(3, 16, 32, 0.55)); transition: filter 0.25s ease; }',
      '.whl-whale:hover .whl-whale-svg { filter: drop-shadow(0 7px 12px rgba(3, 16, 32, 0.55)) drop-shadow(0 0 14px rgba(127, 216, 255, 0.65)); }',
    ...
    const SWIM_PERIOD_S = 70
    const SWIM_START_PX = -170
    const SWIM_TRAVEL_EXTRA_PX = 250

    /**
     * Resume the swim animation from a dropped position. The horizontal phase
     * comes straight from the `whl-swim` keyframes (`-170px → 100vw + 80px`
     * over {@link SWIM_PERIOD_S}) and is expressed as a negative
     * `animation-delay`, so the whale continues from where it was dropped
     * instead of jumping back to the left edge. The vertical lane is kept.
     */
    function resumeSwim(left, top, viewport) {
      const width = viewport && typeof viewport.width === 'number' && Number.isFinite(viewport.width) && viewport.width > 0 ? viewport.width : 0
      const span = width === 0 ? 0 : width + SWIM_TRAVEL_EXTRA_PX
      const raw = span > 0 && Number.isFinite(left) ? (left - SWIM_START_PX) / span : 0
      const progress = Number.isFinite(raw) ? Math.min(1, Math.max(0, raw)) : 0
      return { lane: top, delay: progress === 0 ? 0 : -progress * SWIM_PERIOD_S }
    }

    /** The hover grip pulse is skipped when the user asked for reduced motion. */
    function prefersReducedMotion() {
      if (typeof window === 'undefined' || typeof window.matchMedia !== 'function') return false
      try { return window.matchMedia('(prefers-reduced-motion: reduce)').matches === true } catch (err) { return false }
    }
    ...
    function WhaleApp(props) {
      ...
      const [swim, setSwim] = React.useState({ lane: null, delay: 0, key: 0 })
      const [hovering, setHovering] = React.useState(false)
      const [gripPulse, setGripPulse] = React.useState(false)
      const [hidden, setHidden] = React.useState(() => readHidden())
      const whaleRef = React.useRef(null)
      const dragRef = React.useRef(null)
      const swimKeyRef = React.useRef(0)
      ...
      // Hovering hints grab-ability: the cursor alternates between the open and
      // closed hand (CSS cannot animate `cursor`), unless the user asked for
      // reduced motion.
      React.useEffect(() => {
        if (!hovering || prefersReducedMotion()) { setGripPulse(false); return }
        const timer = setInterval(() => setGripPulse((pulse) => !pulse), 650)
      ...
      const handlePointerDown = (e) => {
        ...
        const rect = node.getBoundingClientRect()
        dragRef.current = { pointerId: e.pointerId, startX: e.clientX, startY: e.clientY, originLeft: rect.left, originTop: rect.top, pos: null }
      ...
      const handlePointerMove = (e) => {
        ...
        const pos = clampWhalePosition(drag.originLeft + dx, drag.originTop + dy, { width: window.innerWidth, height: window.innerHeight })
        drag.pos = pos
        setDragPos(pos)
      }
      const handlePointerUp = (e) => {
        ...
        // A real drag suppresses the synthetic click that follows it, and the
        // whale resumes swimming from exactly where it was dropped.
        suppressClickRef.current = true
        const dropped = drag.pos
        if (dropped) {
          const { lane, delay } = resumeSwim(dropped.left, dropped.top, { width: window.innerWidth, height: window.innerHeight })
          swimKeyRef.current += 1
          setSwim({ lane: lane, delay: delay, key: swimKeyRef.current })
        }
        setDragPos(null)
      }
      if (hidden) {
      ...
      const swimmerStyle = dragPos
        ? { left: dragPos.left + 'px', top: dragPos.top + 'px', animation: 'none' }
        : {
            top: swim.lane === null ? undefined : swim.lane + 'px',
            animationDelay: swim.delay === 0 ? undefined : swim.delay + 's',
          }
      return el('div', { className: phase === 'idle' ? 'whl-frame' : 'whl-frame whl-hold' },
        el('div', { key: swim.key, className: dragPos ? 'whl-swimmer whl-placed' : 'whl-swimmer', style: swimmerStyle },
          el('div', { className: 'whl-bob' },
            el('div', {
              key: sprayId,
              ref: whaleRef,
              className: (sprayId > 0 ? 'whl-whale whl-hop' : 'whl-whale') + (pressed || gripPulse ? ' whl-grip' : ''),
              role: 'button',
              tabIndex: 0,
              'aria-label': '账户余额',
              title: '戳我喷水~（拖动可挪位置）',
              ...
              onLostPointerCapture: () => { dragRef.current = null; movedRef.current = false; setPressed(false) },
              onKeyDown: (e) => {
            ...
            phase !== 'idle' ? el(WhaleBubble, { key: 'b' + sprayId, phase: phase, data: data, onClose: handleClose, shift: bubbleShift, onHide: handleHide }) : null,
          ),
        ),
      )
    }
    ...
    exports.helpers = { clampWhalePosition, prefersReducedMotion, readHidden, resumeSwim, writeHidden, EDGE_MARGIN, SWIM_PERIOD_S, WHALE_H, WHALE_W }
