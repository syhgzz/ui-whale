...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = [
      ...
      '.whl-restore { position: fixed; right: 14px; bottom: 14px; display: flex; align-items: center; justify-content: center; width: 34px; height: 28px; padding: 0; border: 1px solid #2c6da5; border-radius: 10px; background: rgba(12, 36, 64, 0.75); opacity: 0.45; cursor: pointer; pointer-events: auto; }',
    ...
    function WhaleBubble(props) {
      ...
      const close = () => el('button', { className: 'whl-close', onClick: onClose, 'aria-label': '关闭' }, '×')
      const actions = el('div', { className: 'whl-actions' },
        props.placed ? el('button', { className: 'whl-action', onClick: props.onResume }, '继续游动') : null,
        el('button', { className: 'whl-action', onClick: props.onHide }, '隐藏鲸鱼'),
      )
      if (phase === 'loading') {
        return el('div', shell,
          close(),
