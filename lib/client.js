...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function GroupSection(props) {
      ...
      return el('div', { className: 'whl-group' },
        el('div', { className: 'whl-group-head' },
          ...
          el('span', { className: 'whl-group-sub' }, fmtTokens(sub.tokens) + ' tokens · ≈' + fmtCost(sub.cost)),
        ),
        el(ModelRows, { models: props.models, emptyText: props.emptyText }),
      )
    }
      ...
      return el('div', { className: 'whl-bubble' },
        ...
        models.length === 0
          ...
          : el('div', { className: 'whl-rows' },
              el(GroupSection, { title: 'Pro 模型', models: groups.pro, emptyText: '今日无 Pro 模型调用' }),
              el(GroupSection, { title: 'Flash-Vision-Exp', models: groups.vision, emptyText: '今日无 Flash-Vision-Exp 调用' }),
              el(GroupSection, { title: '其他模型', models: groups.other, emptyText: '今日无其他模型调用' }),
            ),
        el('div', { className: 'whl-est' }, '费用按 DeepSeek 公开价格换算为人民币估算，仅供参考。'),
      )
    }
    ...
    function assertUsageShape(value) {
      ...
      if (typeof value.totalCost !== 'number') throw new TypeError('whale/usage result.totalCost must be a number')
      ...
      for (const m of value.models) {
        ...
        for (const key of ['input', 'output', 'cacheRead', 'cacheWrite', 'calls', 'cost']) if (typeof m[key] !== 'number') throw new TypeError('whale/usage model.' + key + ' must be a number')
      }
