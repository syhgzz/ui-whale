...
class WhaleUsageService extends TypertRemoteService {
  constructor(ctx) {
    ...
    // Register the invocations through the host tree's typert registry so the
    // gateway (which reads the tree's own protocol copy) can claim and serve
    // `whale/usage` and `whale/balance` regardless of which package copy
    // libraries resolve.
    ctx.inject(['typert'], (c) => {
      try {
        c.typert.register({
  ...
  /**
   * Balance-only invocation: the billboard asks for the account balance and
   * nothing else, so this path never scans session logs and never queries the
   * platform console. A missing key or a transport failure yields
   * `{ ok: true, balance: null }` — "unknown", never a fabricated zero.
   * @returns the balance snapshot.
   */
  async balance() {
    return { ok: true, balance: await fetchBalance(this.ctx) }
  }
}
